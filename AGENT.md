...
| Intent | Prefer | Avoid |
|--------|--------|-------|
| General web facts / discovery | `brave_web_search` → `brave_llm_context`，或直接 `tavily_search` | 一上来就 SearXNG |
...
2. Prefer Brave/Tavily; use `searxng_search` only as **fallback** when paid/API backends are unavailable or exhausted (upstream blocks are common on self-hosted meta-search).
3. Do not chain SearXNG + Tavily + Brave + Firecrawl for the same query.
